...
void rtw_hal_k(_adapter *adapter, u8 trigger_type)
{
	...
	if (trigger_type == 1) {
		RTW_INFO("[%s] IQK\n", __FUNCTION__);
		halrf_cmn_info_set(p_dm_odm, HALRF_CMNINFO_ABILITY, HAL_RF_IQK);
		do_iqk_8188e(p_dm_odm, 0, thermal_value, 0);
		p_dm_odm->rf_calibrate_info.need_iqk = _FALSE;
	} else if (trigger_type == 2) {
		RTW_INFO("[%s] LCK\n", __FUNCTION__);
		halrf_cmn_info_set(p_dm_odm, HALRF_CMNINFO_ABILITY, HAL_RF_LCK);
		do_lck_8188e(p_dm_odm, thermal_value, 0);
		p_dm_odm->rf_calibrate_info.need_lck = _FALSE;
...
*/
		...
		case RTL8188E :
			/*set TX_AGC_A*/ 
			SET_BITS_TO_LE_4BYTE(desc+24, 0, 5, dpt_lv);
			break;
	#endif
