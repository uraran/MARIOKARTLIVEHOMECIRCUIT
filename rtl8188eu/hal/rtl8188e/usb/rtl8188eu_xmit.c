...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz , u8 bagg_pkt)
{
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		rtw_phydm_set_dyntxpwr(padapter, (u8*)ptxdesc, pattrib->mac_id);
#endif
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if(padapter->registrypriv.vrtl_carrier_sense == ENABLE_VCS)
				ptxdesc->txdw4 &= cpu_to_le32(~HW_RTS_EN);
		...
		ptxdesc->txdw4 &=  cpu_to_le32(~ BIT(15));
		ptxdesc->txdw4 &=  cpu_to_le32(~ BIT(16));
		ptxdesc->txdw4 &=  cpu_to_le32(~ BIT(17));
