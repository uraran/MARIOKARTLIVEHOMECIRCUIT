...
u8 phydm_dtplv2txagc_88e(void *dm_void, u8 tx_pwr_lv)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 ret = 0xff;
	 ...
	 if (tx_pwr_lv == PHYDM_OFFSET_MINUS_7DB)
		return PHYDM_88E_OFFSET_MINUS_7DB;
	else if (tx_pwr_lv == PHYDM_OFFSET_MINUS_3DB)
		return PHYDM_88E_OFFSET_MINUS_3DB;
	else /* PHYDM_OFFSET_ZERO */
		return PHYDM_88E_OFFSET_ZERO;
...
#else
				/* This is for API non-support IC : 92E, 88E */
...
void odm_set_dyntxpwr(void *dm_void, u8 *desc, u8 macid)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dtp_info *dtp = NULL;
	u8 fill_txpwr_offset;
	dtp = &dm->phydm_sta_info[macid]->dtp_stat;
	if (!(dm->support_ability & ODM_BB_DYNAMIC_TXPWR))
		return;
	fill_txpwr_offset = dtp->dyn_tx_power;
#if (RTL8188E_SUPPORT == 1)
	fill_txpwr_offset = phydm_dtplv2txagc_88e(dm_void, fill_txpwr_offset);
#endif
	if (dm->fill_desc_dyntxpwr)
		dm->fill_desc_dyntxpwr(dm, desc, fill_txpwr_offset);
	else
		PHYDM_DBG(dm, DBG_DYN_TXPWR,
