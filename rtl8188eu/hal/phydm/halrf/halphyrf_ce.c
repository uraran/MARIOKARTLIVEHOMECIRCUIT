...
)
{
	...
	if (!(dm->support_ic_type & ODM_RTL8821)) {	/*no PG, do LCK at initial status*/
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "need_iqk= 0x%x, need_lck=0x%x\n", cali_info->need_iqk, cali_info->need_lck);
	...
	if (k) {
		if ((cali_info->pre_need_iqk == 0 && cali_info->need_iqk == 1) ||
			(cali_info->pre_need_lck == 0 && cali_info->need_lck == 1)) {
			if (adapter && ((PADAPTER)adapter)->pid[0] != 0) {
				rtw_signal_process(((PADAPTER)adapter)->pid[0], SIGUSR2);
				printk("signaling... need_iqk = %u, need_lck = %u\n",
					cali_info->need_iqk, cali_info->need_lck);
			}
		}

		cali_info->pre_need_iqk = cali_info->need_iqk;
		cali_info->pre_need_lck = cali_info->need_lck;
	}
}
