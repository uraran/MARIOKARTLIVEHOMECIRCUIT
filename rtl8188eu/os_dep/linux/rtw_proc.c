...
static ssize_t  proc_set_trigger_k(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		u8 trigger_type;
		int num = sscanf(tmp, "%hhu", &trigger_type);

		if (num != 1) {
			RTW_ERR("Invalid argument number.\n");
			return -EFAULT;
		}

		if (!check_fwstate(&adapter->mlmepriv, _FW_LINKED)) {
			RTW_INFO("Driver K. Deny trigger from up layer.\n");
			goto exit;
		}
		if (adapter->k)
			rtw_hal_k(adapter, trigger_type);
		else
			RTW_INFO("Driver K. Deny trigger from up layer.\n");
	}
...
static ssize_t
proc_set_k_pid(struct file *file, const char __user *buffer,
...
static int proc_get_k_pid(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
...
static ssize_t
proc_set_k_enable(struct file *file, const char __user *buffer,
			 size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (k != 0 && k != 1) {
			RTW_ERR("Invalid argument value: %u.\n", k);
			return -EINVAL;
		}

		/* reset need_iqk when change k mode */
		if (adapter->k != k) {
			p_dm_odm->rf_calibrate_info.need_iqk = 0;
			p_dm_odm->rf_calibrate_info.need_lck = 0;
			p_dm_odm->rf_calibrate_info.pre_need_iqk = 0;
			p_dm_odm->rf_calibrate_info.pre_need_lck = 0;
			adapter->k = k;
		} else {
			RTW_INFO("Already in %s_K mode\n",
				k ? "CalControl" : "Driver");
		}
	}
...
static int proc_get_k_enable(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
...
static int proc_get_k_info(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dm_struct *p_dm_odm = adapter_to_phydm(adapter);
	u8 need_iqk = p_dm_odm->rf_calibrate_info.need_iqk;
	u8 need_lck = p_dm_odm->rf_calibrate_info.need_lck;
	u8 thermal_value = p_dm_odm->rf_calibrate_info.avg_thermal_value;
	u8 thermal_value_iqk = p_dm_odm->rf_calibrate_info.thermal_value_iqk;
	u8 thermal_value_lck = p_dm_odm->rf_calibrate_info.thermal_value_lck;

	RTW_PRINT_SEL(m, "need_iqk=%u\n", need_iqk);
	RTW_PRINT_SEL(m, "need_lck=%u\n", need_lck);
	RTW_PRINT_SEL(m, "thermal_value=%u\n", thermal_value);
	RTW_PRINT_SEL(m, "thermal_value_iqk=%u\n", thermal_value_iqk);
	RTW_PRINT_SEL(m, "thermal_value_lck=%u\n", thermal_value_lck);

	return 0;
}

static ssize_t
proc_set_dbg_func_enable(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		u8 dbg_func_en;
		int num = sscanf(tmp, "%hhu", &dbg_func_en);
		...
		if (dbg_func_en != 0 && dbg_func_en != 1) {
			RTW_ERR("Invalid argument value: %u.\n", dbg_func_en);
			return -EINVAL;
		}

		adapter->dbg_func_en = dbg_func_en;
	}

	return count;
}

static int proc_get_dbg_func_enable(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
...
static int proc_get_tsf(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	u64 tsftr;
	...
	if (MLME_IS_ASOC(adapter) == _TRUE) {
		...
		tsftr_l = rtw_read32(adapter, REG_TSFTR);
		tsftr_h = rtw_read32(adapter, REG_TSFTR + 4);
		tsftr_h = tsftr_h << 32;
		tsftr = tsftr_h | tsftr_l;
	} else {
		tsftr = 0;
	}

	RTW_PRINT_SEL(m, "0x%016llx\n", tsftr);

	return 0;
}

static int proc_get_antdiv(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	...
	type = phydm_cmn_info_query(dm, PHYDM_ANTDIV_TYPE);
	RTW_PRINT_SEL(m, "%s\n", type == 0 ? "Auto" :
				 type == 1 ? "Main" :
				 type == 2 ? "Aux" : "NA");
...
static ssize_t
proc_set_abn_stat(struct file *file, const char __user *buffer,
			   size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		u8 err_type;
		int num = sscanf(tmp, "%hhx", &err_type);

		if (num != 1) {
			RTW_ERR("Invalid argument number.\n");
			return -EFAULT;
		}
		if (err_type < 0x1 || err_type > 0x1F) {
			RTW_ERR("Invalid argument value: %u.\n", err_type);
			return -EINVAL;
		}
...
static int proc_get_abn_stat(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *a = (_adapter *)rtw_netdev_priv(dev);

	RTW_PRINT_SEL(m, "usb_bus_error=%u\n", a->usb_bus_err_cnt);
	RTW_PRINT_SEL(m, "tx_dma_error=%u\n", a->txdma_err_cnt);
	RTW_PRINT_SEL(m, "tx_hang_error=%u\n", a->tx_hang_cnt);
	RTW_PRINT_SEL(m, "wait_ack_error=%u\n", a->wait_ack_to_cnt);
	RTW_PRINT_SEL(m, "link_status_error=%u\n", a->link_stat_err_cnt);

	return 0;
}

static ssize_t
proc_set_abn_stat_pid(struct file *file, const char __user *buffer,
			       		size_t count, loff_t *pos, void *data)
{
...
static int proc_get_abn_stat_pid(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);

	RTW_PRINT_SEL(m, "abnormal_stat_pid=%d\n", adapter->pid[1]);
...
const struct rtw_proc_hdl adapter_proc_hdls[] = {
	...
	* 	RTW_PROC_HDL_SSEQ("rssi_disp",proc_get_rssi_disp, proc_set_rssi_disp), */
	...
	RTW_PROC_HDL_SSEQ("EncryptKey", NULL, proc_set_EncryptKey),
	RTW_PROC_HDL_SSEQ("EncryptEnable", proc_get_Encrypt_enable, proc_set_Encrypt_enable),
...
#endif
	RTW_PROC_HDL_SSEQ("CalInfoPid", proc_get_k_pid, proc_set_k_pid),
	RTW_PROC_HDL_SSEQ("CalControlEnable", proc_get_k_enable, proc_set_k_enable),
	RTW_PROC_HDL_SSEQ("CalInfo", proc_get_k_info, NULL),
	RTW_PROC_HDL_SSEQ("CalControl", NULL, proc_set_trigger_k),
	RTW_PROC_HDL_SSEQ("DbgFuncEnable", proc_get_dbg_func_enable, proc_set_dbg_func_enable),
	RTW_PROC_HDL_SSEQ("TsfInfo", proc_get_tsf, NULL),
	RTW_PROC_HDL_SSEQ("AntDivInfo", proc_get_antdiv, NULL),
	RTW_PROC_HDL_SSEQ("ErrorInfo", proc_get_abn_stat, proc_set_abn_stat),
	RTW_PROC_HDL_SSEQ("ErrorInfoPid", proc_get_abn_stat_pid, proc_set_abn_stat_pid),
};
