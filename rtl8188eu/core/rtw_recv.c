...
sint validate_recv_data_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(adapter->Encrypt_en == _TRUE) {

		if ( !(pattrib->swdec) && !(pattrib->macid_vld)) {
			if ((get_frame_sub_type(ptr) & WIFI_DATA_NULL) != WIFI_DATA_NULL) {
				RTW_INFO("%s , drop unencrypted packet in Encrypt mode \n", __FUNCTION__);
				ret = _FAIL;
				goto exit;
			}
		} 
...
exit:
	if (adapter->dbg_func_en &&
	    ret == _SUCCESS && pattrib && pattrib->physt) {
		int msec, sec, hr, min;
		u8 type_tmp = *(u8 *)ptr;

		rtw_gettimeofday(&msec, &sec, &min, &hr);
		printk("NTD_DBG>%02d:%02d:%02d.%03d\t0x%x 0x%x\t%s\t%d\tNA\n",
			hr, min, sec, msec, type_tmp & 0x0C, (type_tmp & 0xF0) >> 4,
			HDATA_RATE(pattrib->data_rate),
			(s8)translate_percentage_to_dbm(pattrib->phy_info.signal_strength));
	}
	return ret;
}
