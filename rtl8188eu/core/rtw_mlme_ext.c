	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network), ETH_ALEN)) {
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				if (padapter->dbg_func_en) {
					rtw_gettimeofday(&msec, &sec, &min, &hr);
					printk("NTD_DBG>%02d:%02d:%02d.%03d\t0x%x 0x%x\t%s\t%d\t0x%016llx\n",
						hr, min, sec, msec, type_tmp & 0x0C, (type_tmp & 0xF0) >> 4,
						HDATA_RATE(attrib->data_rate),
						(s8)translate_percentage_to_dbm(attrib->phy_info.signal_strength), tsf);
				}
...
s32 dump_mgntframe_and_wait_ack_timeout(_adapter *padapter, struct xmit_frame *pmgntframe, int timeout_ms)
{
	...
	if (ret != _SUCCESS)
		abnormal_stat(padapter, R_WAIT_ACK_TO);
...
{
	struct sta_info		*psta = NULL, *psta_bmc;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	WLAN_BSSID_EX		*cur_network = &(pmlmeinfo->network);
	struct sta_priv		*pstapriv = &padapter->stapriv;
...
exit_mlmeext_joinbss_event_callback:
	...
	RTW_INFO("=>%s - End to Connection without 4-way\n", __FUNCTION__);
        ...
        if(psta){
		sta = dm->phydm_sta_info[psta->cmn.mac_id];
		p_ra_info = &(dm->ra_info[sta->mac_id]);
		rtw_write32(padapter, REG_ARFR0, p_ra_info->rate_mask); /*according to rate mask to set the arfr table*/
		RTW_INFO("=>%s - Set ARFR0 (0x444) to limit retry rate\n", __FUNCTION__);

		if(padapter->Encrypt_en)
			mlmeext_set_EncryptKey(padapter, psta);
        }
}

void mlmeext_set_EncryptKey(_adapter * padapter, struct sta_info *psta ){

	struct wiphy *wiphy = adapter_to_wiphy(padapter);
	struct wireless_dev *rtw_wdev = padapter->rtw_wdev;
	struct net_device *ndev = wdev_to_ndev(rtw_wdev);
	struct key_params params;
	u8 aes_seq[6] = {0x00,0x00,0x00,0x00,0x00,0x00};
	
	params.cipher = 0xFAC04;
	params.key = padapter->EncryptKey;
	params.seq = aes_seq;
	params.key_len = 16;
	params.seq_len = 6;

	cfg80211_rtw_add_key(wiphy, ndev, 0, 1, psta->cmn.mac_addr, &params );
	cfg80211_rtw_add_key(wiphy, ndev, 1, 0, psta->cmn.mac_addr, &params );
...
bypass_active_keep_alive:
			...
			if (rx_chk == _FAIL) {
				pmlmeext->retry++;
				abnormal_stat(padapter, R_LINK_STAT_ERR);
	...
	if (survey_channel != 0) {
		...
		if (ScanType == SCAN_ACTIVE) {
...
#endif /* CONFIG_P2P */
			{
				...
				if (pmlmeext->sitesurvey_res.scan_mode == SCAN_ACTIVE && ssid_scan == 0) {
