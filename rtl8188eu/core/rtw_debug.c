...
void abnormal_stat(_adapter *adapter, enum richmomd_ab_stat stat)
{
	switch (stat) {
	case R_USB_ERR:
			adapter->usb_bus_err_cnt++;
			break;
	case R_TXDMA_ERR:
			adapter->txdma_err_cnt++;
			break;
	case R_TXHANG_ERR:
			adapter->tx_hang_cnt++;
			break;
	case R_WAIT_ACK_TO:
			adapter->wait_ack_to_cnt++;
			break;
	case R_LINK_STAT_ERR:
			adapter->link_stat_err_cnt++;
			break;
	default:
			RTW_INFO("Status not support\n");
			break;
	}

	if (adapter->pid[1])
		rtw_signal_process(adapter->pid[1], SIGUSR1);
}
...
static int str2hexbuf_EncryptKey(char *str, u8 *hexbuf)
{
	u8 *p;
	int i, idx = 0;
	...
	p += 2;
	for (i = 0 ; i < 16; i++, idx = idx+2) {
		hexbuf[i] = key_2char2num(p[idx], p[idx + 1]);
	}
	return _TRUE;
}

ssize_t proc_set_EncryptKey(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	if ((check_fwstate(pmlmepriv, _FW_UNDER_LINKING) == _TRUE ) || (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) == _TRUE )
		|| (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)) {

		RTW_INFO("Not allowed to set key \n");
		return count;
	}

	if (count == 35) {
		if (buffer && !copy_from_user(tmp, buffer, count)) {
			str2hexbuf_EncryptKey(tmp, adapter->EncryptKey);
		}
	} else if (count == 17 ) {
		if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", 
			&adapter->EncryptKey[0],&adapter->EncryptKey[1],&adapter->EncryptKey[2],&adapter->EncryptKey[3],
			&adapter->EncryptKey[4],&adapter->EncryptKey[5],&adapter->EncryptKey[6],&adapter->EncryptKey[7],
			&adapter->EncryptKey[8],&adapter->EncryptKey[9],&adapter->EncryptKey[10],&adapter->EncryptKey[11],
			&adapter->EncryptKey[12],&adapter->EncryptKey[13],&adapter->EncryptKey[14],&adapter->EncryptKey[15]);
	    }
	} else {
		
		RTW_INFO("AES setting fails , you need to enter 16 bytes\n");
	}
	
	return count;
}

ssize_t proc_set_Encrypt_enable(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	if ((check_fwstate(pmlmepriv, _FW_UNDER_LINKING) == _TRUE ) || (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) == _TRUE )
		...
		RTW_INFO(" Not allowed to change connection mode \n");
		return count;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (enable) {
			RTW_INFO("Encrypt mode enable \n");
			adapter->Encrypt_en = _TRUE ;
		} else {				
			RTW_INFO("Encrypt mode Disable \n");
			adapter->Encrypt_en = _FALSE ;
		}
	}
...
int proc_get_Encrypt_enable(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);

	RTW_PRINT_SEL(m, "mode=%s\n", adapter->Encrypt_en ? "Encrypt" : "Open");
