...
static loff_t factory_get_start_address(void)
{
	...
	reservedSize += (6+1*4) * mtd->erasesize; //NF profile+BBT=6blocks, hwsetting=1block
	// add extra 20% space for safety.
	reservedSize = rtd_size_aligned(reservedSize*1.2, mtd->erasesize);

	printf("reservedSize:%08x\r\n",reservedSize);
	printf("factory_8MB:%08x\r\n",factory_8MB);
	printf("retrun:%08x\r\n",(reservedSize -factory_8MB));
...
#if 1 // JINN - 20160604 add reading function for workaround	
	rtkspi_read32( (unsigned int)buffer, factory_data_address, sizeof(posix_header));
#endif
	{
		/* Write success */
...
static int factory_save_to_NAND(uchar* buffer, int len)
{
		...
		read_addr = factory_get_start_address();
		printf("factory start offset :%llx\r\n",read_addr);
		if(read_addr==0){
		  printf("read_addr=NULL\r\n");
		  return -1;
		}
		if (buffer == NULL) {
			return -1;
		...
		total_len = rtd_size_aligned(total_len , nand->oobblock);
		
		oob_buf[0] = 0x82;	//block tag--> NAND_BLOCK_FACTORY_SETTING
		
		if(TAR_PADDING_SZ){
		  printf("[FAC] added padding to file of end. size:0x%x\n",TAR_PADDING_SZ);
		  memset((void *)(uintptr_t)(((unsigned int)(uintptr_t) pp_start) + pp_start->rtk_tarsize + sizeof(posix_header)), (0x0), TAR_PADDING_SZ);
		}
		...
		/* remove the RTK header at the end of tar file to prevent side effect */
		memset((void *)(((unsigned int) pp_start) + pp_start->rtk_tarsize), (0x0), 2*sizeof(posix_header));
