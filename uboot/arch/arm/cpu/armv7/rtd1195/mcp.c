...
int AES_CBC_decrypt_key_from_otp(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int iv[4], int key_type)
{
	t_mcp_descriptor *dscpt;
	...
	if(key_type == 0)
		dscpt->mode = 0x201045;	//ks key from OTP
	else if(key_type == 1)
		dscpt->mode = 0x101045;	//kh key from OTP
...
int AES_CBC_decrypt_loop(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int key[4], unsigned int iv[4])
{
	t_mcp_descriptor *dscpt;
