...
#include <asm/arch/rbus/efuse_reg.h>
/*
 * clock enable api
 */
...
#define GPU_speed_ID_bit 3680

int GPU_speed_ID_supported(void)
{
	int offset = GPU_speed_ID_bit%32;

	return rtd_readbits(DATA_115_reg, 1<<offset);
}
/*
...
int clock_init(void)
{
	...
	/*set GPU to 432MHz*/
	if(GPU_speed_ID_supported())
		rtd_outl(PLL_GPU1_reg,0x08607300);
