...
/*	if(boot_mode == BOOT_RESCUE_MODE)
	...
	char *commandline = getenv("bootargs");
	char tmp_cmdline[1024] = {0};

	sprintf(tmp_cmdline, "%s %s %s", commandline, getenv("mtd_part"), rtknand_info);
...
static void boot_jump_linux(bootm_headers_t *images)
{
	...
	writel(0x1, (volatile int*)0xff018000); //to enable the system counter
#endif	
