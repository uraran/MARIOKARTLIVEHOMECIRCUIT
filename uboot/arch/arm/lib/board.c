...
void set_shared_memory_for_communication_with_ACPU(void)
{
	...
	ipc_shm.sys_assign_serial = SWAPEND32(CONFIG_SYS_NS16550_COM1 | 0xb0000000);
...
unsigned int OTP_JUDGE_FINAL_BIT(unsigned int offset)
{
    ...
    real_n = *(volatile unsigned int *)(0x18017000 + align_n);
...
void board_init_r(gd_t *id, ulong dest_addr)
{
	...
	printf("OTP_JUDGE_FINAL_BIT :[0x%x]\n", OTP_JUDGE_FINAL_BIT(OTP_FINALIZE_UART));
	if (OTP_JUDGE_FINAL_BIT(OTP_FINALIZE_UART)) {
		int val_clk;

		val_clk = __raw_readl(0x1800708c);

		val_clk = val_clk & ~(0x1 << 8);
		__raw_writel(val_clk, 0x1800708c);
		val_clk = __raw_readl(0x1800708c);
	}
