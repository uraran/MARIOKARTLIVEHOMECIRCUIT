	...
	for (i = die_size = 0 ; i < NF_CHIP_SEL_NUM; i++)
	{
		...
		for (idx = 0; idx < DEV_SIZE_N; idx++)
		{
			...
			*dev = (void *)&n_device[idx];
			...
			die_size = (UINT32)n_device[idx].size / (UINT32)n_device[idx].num_chips;
...
#if defined (ERASE_WHOLE_NAND_BEFORE_WRITIE) || defined (ERASE_WHOLE_NAND_ONLY)
		prints("erase all.\n");
		prints("blk total:");
		print_hex(blocks_per_flash);
		prints("\n");
...
#if defined (ERASE_WHOLE_NAND_ONLY)
		prints("erase success.\n");
		return 0;	
#endif
#endif
