...
#ifdef AO_ON_SCPU
static int uio_ioctl(struct file *fd, unsigned int cmd, unsigned long args)
{
    printk(KERN_ALERT, " @ %s %d\n", __FUNCTION__, __LINE__);
    int ret = 0, err = 0;
    if (_IOC_TYPE(cmd) != FW_IOC_MAGIC )
    {
        printk(KERN_ALERT " @ %s %d]\n", __FUNCTION__, __LINE__);
        ret = -ENOTTY;
        goto exit;
    }
    if (_IOC_NR(cmd) > AO_IOC_MAXNR)
    {
        printk(KERN_ALERT " @ %s %d]\n", __FUNCTION__, __LINE__);
        ret = -ENOTTY;
        goto exit;
    ...
    switch(cmd)
    {
    case FW_IOC_INIT_RINGBUF_AO:
        printk(KERN_ALERT "FW_IOC_INIT_RINGBUF_AO @ %s\n", __FUNCTION__);
        rtk_alsa_AO_init_ringbuf(args);
        break;
    case FW_IOC_TEST_ION:
        printk(KERN_ALERT "FW_IOC_TEST_ION @ %s\n", __FUNCTION__);
#ifdef ION_TEST_MEMORY_EN
        test_ion_get_fd(args);
#else
        printk(KERN_ALERT "ERR FW_IOC_TEST_ION @ %s\n", __FUNCTION__);
#endif
        break;
    default:
        break;
