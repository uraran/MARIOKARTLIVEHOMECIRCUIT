...
inline void InitMdReg(void)
{
	...
	for(i=0; i<MD_NUM_ENGINES; i++){
		...
		printk("Engine[%d] Ctrl:%08x Base:%08x ReadPtr:%08x WritePtr:%08x Limit:%08x\n",
				i,
...
//	struct md_dev *dev = dev_id;
	...
	//printk("%s:%d interrupt. ...\n", __func__, __LINE__);
	int i;
	for(i=0; i<MD_NUM_ENGINES; i++){
		if(MdRegInfo->MdInte[i].Fields.smq_empty_en && MdRegInfo->MdInts[i].Fields.smq_empty) {
			//dbg_info("md smq empty interrupt");
			//printk("%s:%d md smq empty interrupt. ...\n", __func__, __LINE__);
...
void md_drv_init(struct device *dev)
{
	...
	for(i=0; i<MD_NUM_ENGINES; i++){
		md_hw->engine[i].CmdBuf = virt + MD_CMDBUF_SIZE * i;
...
static int rtk_md_probe(struct platform_device *pdev)
{
	...
	printk("%s: res.start=0x%.8x res.end=0x%.8x\n", __func__,  res.start, res.end);
	printk("%s: mem[%d] phys:0x%08x virt:0x%p size:0x%lx type:%d name:%s\n", __func__, 0, md_uio_info->mem[0].addr, md_uio_info->mem[0].internal_addr, md_uio_info->mem[0].size, md_uio_info->mem[0].memtype, md_uio_info->mem[0].name);
	...
	printk("%s: mem[%d] phys:0x%08x virt:0x%p size:0x%lx type:%d name:%s\n", __func__, 1, md_uio_info->mem[1].addr, md_uio_info->mem[1].internal_addr, md_uio_info->mem[1].size, md_uio_info->mem[1].memtype, md_uio_info->mem[1].name);
	...
	printk("%s: mem[%d] phys:0x%08x virt:0x%p size:0x%lx type:%d name:%s\n", __func__, 2, md_uio_info->mem[2].addr, md_uio_info->mem[2].internal_addr, md_uio_info->mem[2].size, md_uio_info->mem[2].memtype, md_uio_info->mem[2].name);
