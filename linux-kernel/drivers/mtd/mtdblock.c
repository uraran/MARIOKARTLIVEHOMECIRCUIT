			   ...
			   int len, char *buf)
{
	...
	pr_debug("mtdblock: read on \"%s\" at 0x%lx, size 0x%x\n",
			mtd->name, pos, len);
	//printk("[%s] pos:0x%x, len:%d, sect_size:%d\n", __FUNCTION__, pos, len, sect_size);
	...
	while (len > 0) {
		...
		//printk("[%s] sect_start:0x%x, mtdblk->cache_offset:0x%x, mtdblk->cache_state:%d\n", 
		//	__FUNCTION__, sect_start,  mtdblk->cache_offset, mtdblk->cache_state);
			      ...
			      unsigned long block, char *buf)
{
	...
	//printk("[%s] tr->blkshift:%d, tr->blksize:%d \n", __FUNCTION__, tr->blkshift, tr->blksize);
