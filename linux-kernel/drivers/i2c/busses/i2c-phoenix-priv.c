...
#ifdef CONFIG_I2C_PHOENIX_BUS_JAM_RECOVER
    ...
    )
{
    ...
    if (((venus_i2c_flags>>id) & 0x01))
    {
        ...
        printk("[I2C] open I2C%d successed, reference cnt=%d\n", id, atomic_read(&venus_i2c_phy_handle[id]->ref_cnt));
    ...
    if (hHandle!= NULL)
    {
        ...
        printk("[I2C] open I2C%d successed, reference cnt=%d\n", id, atomic_read(&hHandle->ref_cnt));
