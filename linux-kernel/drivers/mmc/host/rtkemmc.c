...
RET_CMD:
	...
	if (gPreventRetry)
	{
        	printk(KERN_WARNING "[LY]ignore error when card in tuning state, err=%d\n",gCurrentBootMode,err);
		return err;
	}
...
static void rtksd_set_ios(struct mmc_host *host, struct mmc_ios *ios)
{
    ...
    if (ios->bus_mode == MMC_BUSMODE_PUSHPULL){
        ...
        if(ios->clock > 400000)
        {
            rtkcr_set_div(sdport,EMMC_CLOCK_DIV_NON);
            rtkcr_set_speed(sdport, 0);
            printk("[LY] rtkemmc : high speed, bus=%d\n", busmode);
        }
        else if(ios->clock > 200000)
        {
            rtkcr_set_div(sdport,EMMC_CLOCK_DIV_128);  
            rtkcr_set_speed(sdport, 2);
            printk("[LY] rtkemmc : mid speed, bus=%d\n", busmode);
        }
        else
        {
            rtkcr_set_div(sdport,EMMC_CLOCK_DIV_128); 
            rtkcr_set_speed(sdport, 2);
            printk("[LY] rtkemmc : low speed, bus=%d\n", busmode);
        }
#endif
       	//rtkcr_set_div(sdport,EMMC_CLOCK_DIV_NON);
    }
...
//#define FAKE_AC_DET_PIN
...
#endif
    {
        ...
        case PCB_PIN_TYPE_ISO_GPIO:
            ...
            if(st_gpio_map[rtk_gpio_num] == 0xff ||
