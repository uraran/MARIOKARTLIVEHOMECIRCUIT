...
static bool optee_msg_api_uid_is_optee_api(optee_invoke_fn *invoke_fn)
{
	...
	invoke_fn(OPTEE_SMC_CALLS_UID, 0, 0, 0, 0, 0, 0, 0, &res, NULL);
...
static bool optee_msg_api_revision_is_compatible(optee_invoke_fn *invoke_fn)
{
	...
	invoke_fn(OPTEE_SMC_CALLS_REVISION, 0, 0, 0, 0, 0, 0, 0, &res, NULL);
					    ...
					    u32 *sec_caps)
{
	...
	invoke_fn(OPTEE_SMC_EXCHANGE_CAPABILITIES, a1, 0, 0, 0, 0, 0, 0, &res, NULL);
			...
			void __iomem **ioremaped_shm)
{
	...
	invoke_fn(OPTEE_SMC_GET_SHM_CONFIG, 0, 0, 0, 0, 0, 0, 0, &res, NULL);
	if (res.a0 != OPTEE_SMC_RETURN_OK) {
...
#ifdef LINUX_KERNEL_V310
	va = ioremap_cached(paddr, size);
#else
...
static int get_invoke_func(struct device *dev, optee_invoke_fn **invoke_fn)
{
	...
	if (!strcmp("hvc", method)) {
		*invoke_fn = __arm_smccc_hvc;
	} else if (!strcmp("smc", method)) {
		*invoke_fn = __arm_smccc_smc;
	} else {
