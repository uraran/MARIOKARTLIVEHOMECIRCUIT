...
static void qtd_list_free (struct ehci_hcd *ehci, struct urb *urb,
...
hub_qh_urb_transaction (
	...
	if (usb_pipecontrol (urb->pipe)) {
		...
		qtd->hw_alt_next = EHCI_LIST_END(ehci); //|  cpu_to_hc32 (ehci, 0x0a);  //hcy added
	 ...
	 */
	qtd_fill(ehci, qtd, buf, 18, backup_token,
			maxpacket);
	qtd->hw_next = EHCI_LIST_END(ehci);
	qtd->hw_alt_next = EHCI_LIST_END(ehci); //|  cpu_to_hc32 (ehci, 0x0a);  //hcy added
	...
	/*if (likely (urb->transfer_buffer_length != 0))*/ {
		...
		if (one_more) {
			...
			qtd_fill(ehci, qtd, 0, 0, backup_token, 0);
			qtd->hw_next = EHCI_LIST_END(ehci);
			qtd->hw_alt_next = EHCI_LIST_END(ehci); //|  cpu_to_hc32 (ehci, 0x0a);  //hcy added
...
static int ehci_init(struct usb_hcd *hcd)
{
	...
	hw->hw_alt_next = QTD_NEXT(ehci, ehci->async->dummy->qtd_dma);// |  cpu_to_hc32 (ehci, 0x0a);  //hcy added
