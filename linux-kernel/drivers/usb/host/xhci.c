...
extern int xhci_queue_ctrl_tx_for_get_desc(struct xhci_hcd *xhci, gfp_t mem_flags,
...
int xhci_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	if (usb_endpoint_xfer_control(&urb->ep->desc)) {
		...
		if (xhci->xhc_state & XHCI_STATE_DYING)
			goto dying;
#ifdef CONFIG_USB_XHCI_HCD_TEST_MODE
 		if(get_UsbDescriptor_index() != 0)
 		{
 			gUsbGetDescriptor_flag = 1;
 			ret = xhci_queue_ctrl_tx_for_get_desc(xhci, GFP_ATOMIC, urb,
 					slot_id, ep_index);
 		} else {
