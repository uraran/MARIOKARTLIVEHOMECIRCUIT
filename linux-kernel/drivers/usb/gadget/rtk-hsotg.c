				      ...
				      struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->wLength == 0) {
		hsotg->ep0_state = EP0_STATUS_IN;
	} else if (ctrl->bRequestType & USB_DIR_IN) {
		hsotg->ep0_state = EP0_DATA_IN;
	} else {
		hsotg->ep0_state = EP0_DATA_OUT;
	}
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	 ...
	 */
	ptxfifo = readl(hsotg->regs + DPTXFSIZn(index));
	hs_ep->fifo_size = DPTXFSIZn_DPTxFSize_GET(ptxfifo);
	dev_dbg(hsotg->dev, "%s: %s fifo_size=%d\n",
		    __func__, hs_ep->name, hs_ep->fifo_size);
...
#else
	writel(readl(IOMEM(0xfe01380c))|BIT(0), IOMEM(0xfe01380c));
	mdelay(100);
	writel(readl(IOMEM(0xfe01382c))|BIT(0), IOMEM(0xfe01382c));
	mdelay(100);

//	writel(readl(IOMEM(0xfe013828))&~BIT(30), IOMEM(0xfe013828));	//barry ??
//	writel(readl(IOMEM(0xfe013828))|BIT(0), IOMEM(0xfe013828));	//barry ??

	writel(readl(IOMEM(0xfe013840))|BIT(0), IOMEM(0xfe013840));	//enable OTG
	mdelay(100);
	writel(readl(IOMEM(0xfe013844))|0x180,  IOMEM(0xfe013844));
	mdelay(100);
#endif
}
 ...
 */
int	rtk_usb_phy_init(void);				//barry
