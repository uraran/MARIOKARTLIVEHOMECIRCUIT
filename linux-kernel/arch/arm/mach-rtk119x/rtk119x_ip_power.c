...
static void __rtd_clearbits(unsigned int addr, unsigned int mask)
{
	unsigned int v;
	unsigned int i;

	eprintk("%s: %08x: before clear bit val=%08x, mask=%08x\n", __func__, addr, READ_REG_INT32U(addr), mask);
	wmb();
#if 0
	v = READ_REG_INT32U(addr);
	eprintk("%s: %08x: v=%08x\n", __func__, addr, v);
	wmb();
	...
	for (i = 0; i < 32; i++) {
		if (v & BIT(i)) {
			eprintk("%s: %08x: clear bit %d\n", __func__, addr, i);
			rtd_clearbits(addr, BIT(i));
		}
	}
#else
	rtd_clearbits(addr, mask);
#endif
	eprintk("%s: %08x: after clear bits val=%08x\n", __func__, addr, READ_REG_INT32U(addr));
	wmb();
}

static int __init crt_reg_init(void)
{
	__rtd_clearbits(0x18000000, ~0x6b868b7d);
	__rtd_clearbits(0x18000004, ~0x14800203);
	__rtd_clearbits(0x1800000C, ~0x08ac247b);
	__rtd_clearbits(0x18000010, ~0x00FF2034);
	return 0;
}
late_initcall(crt_reg_init);
