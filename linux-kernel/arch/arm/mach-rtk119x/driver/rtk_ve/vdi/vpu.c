...
#endif
	printk("[VPUDRV] base:0x%lx, phy_addr:0x%lx, size:%d\n", vb->base, (unsigned long)vb->phys_addr, vb->size);
	return 0;
}
	...
	list_for_each_entry_safe(vil, n, &s_inst_list_head, list) 
	{
		if (vil->filp == filp)
		{
			...
			if (vip)
			{
				...
				{
					printk(KERN_WARNING "[VPUDRV] vil->inst_idx:%ld, vil->core_idx:%ld is pending, clear in here\n", vil->inst_idx, vil->core_idx);
					vip->pendingInst = 0;
...
static int vpu_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	VPU_WRITE_REG_INT32U(VPU_CLOCK_ENABLE_REG, clk_en_val);
	__delay(100);
#elif defined VPU_SUPPORT_CLOCK_CONTROL
	vpu_clk_enable(s_vpu_clk_980);
	vpu_clk_enable(s_vpu_clk_410);
#endif
