...
#define ISO_WATCH_BASE				(0xfe007680) // 0x18007680
...
int rtk_watchdog_kick( unsigned int msec )
{
	...
	if( msec == 0xdeadbeef ) {
		...
		writel(1, IOMEM(ISO_WATCH_BASE+ISO_WDT_CLR));
		writel(0x0080beef, IOMEM(ISO_WATCH_BASE+ISO_WDT_OVERFLOW));
		writel(0xff, IOMEM(ISO_WATCH_BASE+ISO_WDT_CTL));
		kick_ignore = 1;
	}
	else {
		if( !kick_ignore ) {
			printk("kick watchdog threshold (%d ms)\n", msec);
			writel(1, IOMEM(ISO_WATCH_BASE+ISO_WDT_CLR));
			writel((msec*ISO_WDT_1MS_CNT), IOMEM(ISO_WATCH_BASE+ISO_WDT_OVERFLOW));
			writel(0xff, IOMEM(ISO_WATCH_BASE+ISO_WDT_CTL));
		}
	}
...
static int rtk_watchdog_disable( void )
{
	...
	writel(0xA5, IOMEM(ISO_WATCH_BASE+ISO_WDT_CTL));
	kick_ignore = 1;
...
int __init rtk_watchdog_init(void)
{ 
    printk(KERN_INFO "rtk watchdog driver init\n");
...
void __exit rtk_watchdog_exit(void)
{
...
module_init(rtk_watchdog_init);
module_exit(rtk_watchdog_exit);
