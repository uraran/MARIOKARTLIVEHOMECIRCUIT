...
typedef struct{
    ...
    unsigned int RefClockAddr;
    unsigned int ClockAddr_HI;
    unsigned int ClockAddr_LOW;
...
#define DC_SHORT_FENCE_TIMEOUT  (33)
#define DC_LONG_FENCE_TIMEOUT   (50)

enum dc_buffer_id {
    eFrameBuffer            = 0x1U << 0,
    eFrameBufferSkip        = 0x1U << 0,
    eIONBuffer              = 0x1U << 1,
    eUserBurrer             = 0x1U << 2,
};
...
union _fence_data{
    int                     fence_fd;   /* user   space : fenceFd (input) */
 ...
 */
#if 0 // NO USE
#define SE_VO_SYNC_ADDR  (0xA0000100)
#endif
#define DCRT_IRQ                (65) /* AVCPU_NWC : AVCPU => SCPU */
#define DC_NOHW_MAX_BACKBUFFERS (4)

#define DC_VO_SYNC_ADDR         (0x0000b104) // (0xA0000104)
#define dc_endian_swap_32(a)    (((a)>>24) | (((a)>>8) & 0xFF00) | (((a)<<8) & 0xFF0000) | (((a)<<24) & 0xFF000000))
#define DC_VO_SET_NOTIFY        (dc_endian_swap_32(1L << 0))
#define DC_VO_FEEDBACK_NOTIFY   (dc_endian_swap_32(1L << 1))
#define RPC_AUDIO_SET_NOTIFY        (dc_endian_swap_32(1L << 8))
#define RPC_AUDIO_FEEDBACK_NOTIFY   (dc_endian_swap_32(1L << 9))
#define DC_HAS_BIT(addr, bit)   ((*(volatile uint32_t *)addr) &(bit))
#define DC_SET_BIT(addr,bit)    ((*(volatile uint32_t *)addr) |= (bit))
#define DC_RESET_BIT(addr,bit)  ((*(volatile uint32_t *)addr) &= (~(bit)))

int     DC_Ioctl        (struct fb_info *fbi, VENUSFB_MACH_INFO * video_info,unsigned int cmd, unsigned int arg);
